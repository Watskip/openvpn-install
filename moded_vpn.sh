...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	echo
	echo "Creating the first client:"
	client="client"
	echo
...
tls-crypt tc.key
topology subnet
server 192.168.252.0 255.255.255.0" > /etc/openvpn/server/server.conf
	# IPv6
	if [[ -z "$ip6" ]]; then
...
crl-verify crl.pem" >> /etc/openvpn/server/server.conf
	...
	if systemctl is-active --quiet firewalld.service; then
		...
		firewall-cmd --zone=trusted --add-source=192.168.252.0/24
		firewall-cmd --permanent --add-port="$port"/"$protocol"
		firewall-cmd --permanent --zone=trusted --add-source=192.168.252.0/24
		# Set NAT for the VPN subnet
		firewall-cmd --direct --add-rule ipv4 nat POSTROUTING 0 -s 192.168.252.0/24 ! -d 192.168.252.0/24 -j SNAT --to "$ip"
		firewall-cmd --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -s 192.168.252.0/24 ! -d 192.168.252.0/24 -j SNAT --to "$ip"
		if [[ -n "$ip6" ]]; then
...
[Service]
Type=oneshot
ExecStart=$iptables_path -w 5 -t nat -A POSTROUTING -s 192.168.252.0/24 ! -d 192.168.252.0/24 -j SNAT --to $ip
ExecStart=$iptables_path -w 5 -I INPUT -p $protocol --dport $port -j ACCEPT
ExecStart=$iptables_path -w 5 -I FORWARD -s 192.168.252.0/24 -j ACCEPT
ExecStart=$iptables_path -w 5 -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$iptables_path -w 5 -t nat -D POSTROUTING -s 192.168.252.0/24 ! -d 192.168.252.0/24 -j SNAT --to $ip
ExecStop=$iptables_path -w 5 -D INPUT -p $protocol --dport $port -j ACCEPT
ExecStop=$iptables_path -w 5 -D FORWARD -s 192.168.252.0/24 -j ACCEPT
	...
	case "$option" in
		...
		3)
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				if systemctl is-active --quiet firewalld.service; then
					ip=$(firewall-cmd --direct --get-rules ipv4 nat POSTROUTING | grep '\-s 192.168.252.0/24 '"'"'!'"'"' -d 192.168.252.0/24' | grep -oE '[^ ]+$')
					# Using both permanent and not permanent rules to avoid a firewalld reload.
					firewall-cmd --remove-port="$port"/"$protocol"
					firewall-cmd --zone=trusted --remove-source=192.168.252.0/24
					firewall-cmd --permanent --remove-port="$port"/"$protocol"
					firewall-cmd --permanent --zone=trusted --remove-source=192.168.252.0/24
					firewall-cmd --direct --remove-rule ipv4 nat POSTROUTING 0 -s 192.168.252.0/24 ! -d 192.168.252.0/24 -j SNAT --to "$ip"
					firewall-cmd --permanent --direct --remove-rule ipv4 nat POSTROUTING 0 -s 192.168.252.0/24 ! -d 192.168.252.0/24 -j SNAT --to "$ip"
